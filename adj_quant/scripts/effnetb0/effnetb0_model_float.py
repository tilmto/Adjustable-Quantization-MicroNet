...
class EffNetB0ModelFloat(object):
    ...
    def _swish_input(self, net):
        net = tf.identity(net, "swish_input")
        self._add_reference_node(net)

        return net
    
    def _swish(self, net):
        num_param = self._get_param_num(net)
        self.num_swish += num_param
        self.num_swish_list.append(num_param)
        return net*tf.nn.sigmoid(net)

    def _get_param_num(self, net):
        sz = 1
        for dim in net.shape[1:]:
            sz *= dim.value
        return sz
    ...
    def _expanded_conv(self, net, cell_scope, strides=1):
        ...
        with tf.name_scope(cell_scope):
            ...
            with tf.name_scope('se_1'):
                ...
                net_se = self._swish_input(net_se)
