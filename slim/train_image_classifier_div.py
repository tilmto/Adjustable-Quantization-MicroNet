      ...
      with tf.Session(config=config) as sess:
        ...
        for epoch_id in range(FLAGS.max_epoch):
          for batch_id in range(num_step_per_epoch):
            loss, global_step_out = sess.run([train_tensor,global_step])
            if not batch_id % 10:
              tf.logging.info('Global step: %d, Training loss: %f,' % (global_step_out,loss))
            if not batch_id % 300:
            ...
            if FLAGS.swa_delay is not None:
              if FLAGS.swa_delay <= epoch_id and not batch_id % int(FLAGS.num_epoch_per_swa*num_step_per_epoch-1) and batch_id:
                weight_current = sess.run(weight_names)
                weight_all_epoch.append(weight_current)
